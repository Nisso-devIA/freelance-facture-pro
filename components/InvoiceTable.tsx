      ...
      case 'draft':
        return (
          <span className="inline-flex items-center gap-1 text-zinc-400">
            <AlertCircle size={18} /> Brouillon
          </span>
        )
      case 'overdue':
        return (
          <span className="inline-flex items-center gap-1 text-red-400">
            <AlertCircle size={18} /> En retard
          </span>
        )
      default:
        return <span className="text-amber-400">Inconnu</span>
    }
  }
    ...
    <div className="bg-zinc-900/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 mt-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
          onClick={onRefresh} 
          disabled={loading}
          className="text-blue-400 hover:text-white flex items-center gap-2 px-4 py-2 rounded-2xl hover:bg-white/5 transition"
        >
          ↻ Actualiser
        </button>
      </div>
      ...
      {loading ? (
        <div className="text-center py-16 text-zinc-500">Chargement des factures...</div>
      ) : invoices.length === 0 ? (
              ...
              {invoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-white/5 transition">
                  ...
                  <td className="py-4 text-center flex justify-center gap-4">
                    ...
                    <button 
                      onClick={() => {
                        // TODO : delete invoice (on l’ajoutera plus tard si tu veux)
